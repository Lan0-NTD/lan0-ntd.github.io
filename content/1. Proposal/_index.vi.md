...
- Các SME thiếu chuyên môn pháp lý nội bộ để rà soát hợp đồng.  
- Việc kiểm tra hợp đồng thủ công chậm, thiếu nhất quán và dễ sai sót.  
- Chi phí tư vấn pháp lý cao, khó mở rộng quy mô đánh giá tài liệu.

### Giải pháp

Một nền tảng SaaS hỗ trợ bởi AI có khả năng:

- Tự động trích xuất và phân loại điều khoản hợp đồng.  
- Làm nổi bật rủi ro pháp lý tiềm ẩn kèm ngữ cảnh trích dẫn.  
- Gợi ý điểm thương lượng và tạo phiên bản hợp đồng sửa đổi.  
- Hỗ trợ soạn hợp đồng dựa trên mẫu (template).
...
- Frontend (Amplify + S3): Giao diện tải hợp đồng, hiển thị rủi ro và tạo báo cáo.  
- API Gateway + Cognito: Xác thực và quản lý truy cập API. 
- EC2 (n8n Orchestrator): Chạy các workflow mẫu (WT-01 → WT-07), tải và chuẩn hóa tệp vào S3.  
- RDS + DynamoDB: Lưu metadata quan hệ và embedding.  
- AI API Integrations: Cung cấp logic trích xuất và suy luận văn bản.  
- CloudWatch + Secrets Manager: Giám sát, xoay khóa và bảo vệ thông tin nhạy cảm.
...
- Triển khai môi trường n8n trên EC2.  
- Tích hợp API LLM bên ngoài cho OCR và phân tích hợp đồng.  
- Dùng DynamoDB local cho lưu trữ embedding.  
- Mô phỏng API Gateway để kiểm thử workflow end-to-end.
...
- Thiết lập cảnh báo CloudWatch và thông báo qua webhook n8n.
...
- Giúp SMEs tiếp cận công nghệ AI trong phân tích hợp đồng với chi phí hợp lý.  
- Giảm phụ thuộc vào nguồn lực tư vấn pháp lý bên ngoài.  
- Đặt nền tảng cho hệ thống quản trị hợp đồng cấp doanh nghiệp trong tương lai.
